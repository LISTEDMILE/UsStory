...
export default function Header() {
  ...
  return (
    <header className="premiumHeader sticky top-0 left-0 w-full z-50 backdrop-blur-2xl bg-black/30 border-b border-white/10">
      <div className="w-full h-15 py-3 px-16 flex justify-between items-center">
        {/* Logo */}
        <img src={'/logo.png'} className="h-full w-auto"/>
      ...
      {open && (
        <div className=" bg-black/80 backdrop-blur-xl border-t border-white/10">
          <div className="fixed h-screen w-screen bg-black/80 inset-0 z-0"
          onClick={()=> setOpen(false)}/>
          <div className="fixed min-h-screen right-0 top-0 w-screen md:w-fit min-w-[300px] flex flex-col py-12 px-8 gap-6 text-white/70 text-sm z-10 bg-gradient-to-b from-[#0f0f14] via-[#151524] to-[#1c1c2e] ">
            ...
            <div className={`hidden md:flex items-center gap-6 ${isLoggedIn ? "self-end" : "self-start"} mt-12`}>
              {isLoggedIn ? (
                <>
                  <button
